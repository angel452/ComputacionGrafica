...
float radiusCamera = 2.0f; // --> Radio de la camara
...
void processInput(GLFWwindow *window){
    ...
    if(glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS){
        angleCameraV = angleCameraV + 0.001f;
    ...
    if(glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS){
        angleCameraH = angleCameraH - 0.001f;
    ...
    if(glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS){
        radiusCamera = radiusCamera - 0.01f;
    }
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window)){
        ...
        if(cameraPos.y > proxNumberChange || cameraPos.y < -proxNumberChange){
            // --> Invertimos el vector up para que la camara no se voltee
            cameraUp = -cameraUp;
        }
