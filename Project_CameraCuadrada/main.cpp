...
glm::vec3 cameraPos = glm::vec3(0.0f, 0.0f, 2.0f); // --> Vector - Posicion de la camara
...
float radiusCamera = 10.0f; // --> Radio de la camara
float angleCameraH = 0.0f; // --> Angulo de la camara en el eje horizontal
float angleCameraV = 0.0f; // --> Angulo de la camara en el eje vertical

float spaceX = 0.0f;
float spaceY = 0.0f;
float spaceZ = 0.0f;
bool rotacionVerticalY = true;
bool rotacionVerticalZ = true;
int stateVert = 1;
int stateHor = 1;
bool rotacionHorizontalX = true;
bool rotacionHorizontalZ = false;
...
void processInput(GLFWwindow *window){
    ...
    if(glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS){
        angleCameraV = angleCameraV + 0.01f;
    
        /*
        float camPosY = cos(angleCameraV) * radiusCamera;
        float camPosZ = sin(angleCameraV) * cos(angleCameraH) * radiusCamera;
    ...
    if(glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS){
        angleCameraH = angleCameraH - 0.01f;

        /*
        float camPosX = sin(angleCameraH) * radiusCamera;
        float camPosZ = cos(angleCameraH) * radiusCamera;
    ...
    if(glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS){
        radiusCamera = radiusCamera - 0.1f;
    }
    ...
    //float proxNumberChange = radiusCamera - 0.001f;
    float mY = cameraPos.y - 2;
    float mY1 = cameraPos.y + 2;

    if(mY > -0.001 && mY < 0.001 && cameraPos.z > -0.001 && cameraPos.z < 0.001){
        cameraUp = -cameraUp;
        cout << "Invert\n"; 
    } else if(mY1 > -0.001 && mY1 < 0.001 && cameraPos.z > -0.001 && cameraPos.z < 0.001){
        cameraUp = -cameraUp;
        cout << "Invert 2\n"; 
    }
...
glm::vec3 cubePositions2[] = {
    ...
    // Capa Superior...
    glm::vec3(  0.00f,  1.02f,  1.02f), // Delantera
    glm::vec3(  1.02f,  1.02f,  1.02f), // Delantera
    glm::vec3( -1.02f,  1.02f,  1.02f), // Delantera

    glm::vec3(  0.00f,  1.02f,  0.00f), // Medios
    glm::vec3(  1.02f,  1.02f,  0.00f), // Medios
    glm::vec3( -1.02f,  1.02f,  0.00f), // Medios

    glm::vec3(  0.00f,  1.02f,  -1.02f), // Traseras
    glm::vec3(  1.02f,  1.02f,  -1.02f), // Traseras
    glm::vec3( -1.02f,  1.02f,  -1.02f), // Traseras

    // Capa Media...
    glm::vec3(  0.00f,  0.0f,  1.02f), // Delantera
    glm::vec3(  1.02f,  0.0f,  1.02f), // Delantera
    glm::vec3( -1.02f,  0.0f,  1.02f), // Delantera

    glm::vec3(  1.02f,  0.0f,  0.00f), // Medios
    glm::vec3( -1.02f,  0.0f,  0.00f), // Medios

    glm::vec3(  0.00f,  0.0f,  -1.02f), // Traseras
    glm::vec3(  1.02f,  0.0f,  -1.02f), // Traseras
    glm::vec3( -1.02f,  0.0f,  -1.02f), // Traseras
    ...
    // Cubos de referencia en los 4 lados
    glm::vec3(  0.00f,  0.0f,  3.04f), // Delantera
    glm::vec3(  3.04f,  0.0f,  0.00f), // Derecha
    glm::vec3(  0.00f,  0.0f,  -3.04f), // Trasera
    glm::vec3(  -3.04f,  0.0f,  0.00f), // Izquierda
};
...
int main()
{
    ...
    */
    ...
    glm::mat4 projection = glm::perspective(glm::radians(fov), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
                            ...
                            //              * Plano mas cercano, 
                            //              * Plano mas lejano )
    // !!! OJO: Ponerla dentro del bucle si dicha matriz no cambia. Despues del shaderProgram.Activar_ShaderProgram(); y antes de la matriz "view"
    ...
    while (!glfwWindowShouldClose(window)){
        ...
        // * Actualizamos la posicion de la camara segun el radio y el angulo
        //float camPosX = radiusCamera * sin(angleCameraH) * sin(angleCameraV);
        //float camPosY = radiusCamera * cos(angleCameraV);
        //float camPosZ = radiusCamera * cos(angleCameraH) * sin(angleCameraV);
        ...
        view = glm::lookAt(cameraPos, glm::vec3(0.0f, 0.0f, 0.0f), cameraUp);   // Para forzar a mirar al origen (0, 0, 0) 
        //view = glm::lookAt(cameraPos, cameraPos + cameraFront, cameraUp);     // Para mirar hacia el vector direccion actual de la camara
